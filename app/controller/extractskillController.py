...
async def extractskills(request: dict):
    try:
        ...
        jobs_skill_data = skill_extractors_utils(jobs_data)
 
        
        result = {
            "status": "success",
            "data": jobs_skill_data
        }
        return result
    ...
    try:
        ...
        for job in jobs_data:
            try:
                ...
                if extracted_skills is None:
                    print(f"Warning: extracted_skills is None for job_id: {job_id}")
                    job_error.append({"job_id": job_id, "error": "Missing extracted_skills"})
                    continue
                ...
                # ตรวจสอบว่ามีทักษะส่งมาหรือไม่
                if len(hard_skills) == 0 and len(soft_skills) == 0:
                    error_message = "No skills provided for insertion"
